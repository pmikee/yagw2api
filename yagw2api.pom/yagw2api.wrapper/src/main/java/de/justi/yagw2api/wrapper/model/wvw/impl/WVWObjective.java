...
final class WVWObjective extends AbstractHasChannel implements IWVWObjective {
	...
	private static final IWVWModelEventFactory WVW_MODEL_EVENTS_FACTORY = YAGW2APIWrapper.getInjector().getInstance(IWVWModelEventFactory.class);
	...
	@Override
	public Optional<String> getLabel() {
		return this.location.getLabel(YAGW2APIWrapper.getCurrentLocale());
	}
	...
	public void claim(IGuild guild) {
		if ((guild != null) && (!this.claimedByGuild.isPresent() || !this.claimedByGuild.get().equals(guild))) {
			...
			LOGGER.info(this.getLocation().getLabel(YAGW2APIWrapper.getCurrentLocale()) + " has been claimed by: " + guild.getName() + " | previous claimed by " + this.claimedByGuild);
			...
			LOGGER.info(this.getLocation().getLabel(YAGW2APIWrapper.getCurrentLocale()) + " has been unclaimed from " + this.claimedByGuild);
