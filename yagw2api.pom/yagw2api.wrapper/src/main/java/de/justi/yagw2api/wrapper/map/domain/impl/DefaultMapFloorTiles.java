...
final class DefaultMapFloorTiles implements MapFloorTiles {
	...
	private final Supplier<Tuple2<Integer, Integer>> floorTextureSize = Suppliers.memoize(new Supplier<Tuple2<Integer, Integer>>() {
		...
		public Tuple2<Integer, Integer> get() {
			final com.google.common.base.Optional<MapFloorDTO> optionalFloorDTO = DefaultMapFloorTiles.this.mapFloorService.retrieveMapFloor(DefaultMapFloorTiles.this.continentId,
					DefaultMapFloorTiles.this.floorIndex);
	...
	private final LoadingCache<Tuple3<Integer, Integer, Integer>, MapTile> tileCache = CacheBuilder.newBuilder().build(
			new CacheLoader<Tuple3<Integer, Integer, Integer>, MapTile>() {
				...
				public MapTile load(final Tuple3<Integer, Integer, Integer> key) throws Exception {
					...
					if (DefaultMapFloorTiles.this.isTileAvailable(key.v2(), key.v3(), key.v1())) {
						...
						return DefaultMapFloorTiles.this.mapDomainFactory.newMapUnavailableTileBuilder().continentId(DefaultMapFloorTiles.this.continentId)
								.floorIndex(DefaultMapFloorTiles.this.floorIndex).zoom(key.v1()).position(Tuples.of(key.v2(), key.v3())).build();
					}
				}
			});
	...
	@Override
	public String toString() {
		return MoreObjects.toStringHelper(this).add("continentId", this.continentId).add("floorIndex", this.floorIndex).add("textureDimension", this.getTextureDimension())
				.toString();
	}
	...
	public Tuple2<Integer, Integer> getTileIndexDimension(final int zoom) {
		return DefaultMapFloorTiles.this.texture2Tile(DefaultMapFloorTiles.this.getTextureDimension(), zoom);
	}

	@Override
	public int getTileTextureSize(final int zoom) {
		checkArgument(zoom >= 0 && zoom <= 7, "invalid zoom: %s", zoom);
		return TILE_SIZE * (int) Math.pow(2, 7 - zoom);
	}
}
